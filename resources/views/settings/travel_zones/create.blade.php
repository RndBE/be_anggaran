<x-app-layout>
    <x-slot name="header">
        <div class="flex justify-between items-center">
            <h2 class="font-semibold text-xl text-gray-800 leading-tight">Tambah Travel Zone Baru</h2>
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            ...
                            <x-text-input id="zone" name="zone" type="number" min="1" class="mt-1 block w-full"
                                value="{{ old('zone', $nextZone) }}" required autofocus />
                            <p class="mt-1 text-xs text-gray-400">Harus unik. Misal: 1, 2, 3, 4…</p>
                        </div>
                        <div>
                            <x-input-label for="name" :value="__('Nama Zone')" />
                            <x-text-input id="name" name="name" type="text" class="mt-1 block w-full"
                                value="{{ old('name') }}" required placeholder="e.g., Jabodetabek" />
                        </div>
                    </div>
                    <div>
                        ...
                        <div class="mt-1 relative">
                            ...
                            <x-text-input id="meal_allowance" name="meal_allowance" type="number" step="1000" min="0"
                                class="block w-full pl-10" value="{{ old('meal_allowance') }}" required
                                placeholder="0" />
                        </div>
                    </div>
