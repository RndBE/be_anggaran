<x-app-layout>
    <x-slot name="header">
        <div class="flex justify-between items-center">
            <h2 class="font-semibold text-xl text-gray-800 leading-tight">Tambah Client Code Baru</h2>
                ...
                <form method="POST" action="{{ route('settings.client-codes.store') }}" class="space-y-4">
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            ...
                            <x-text-input id="prefix" name="prefix" type="text" class="mt-1 block w-full"
                                value="{{ old('prefix') }}" required autofocus placeholder="e.g. GOV1, POE" />
                        ...
                        <div>
                            ...
                            <x-text-input id="instansi_singkat" name="instansi_singkat" type="text"
                                class="mt-1 block w-full" value="{{ old('instansi_singkat') }}" required
                                placeholder="e.g. BBWS, BRANTAS" />
                        </div>
                    </div>
                    ...
                    <div>
                        ...
                        <x-text-input id="counter" name="counter" type="number" min="0" class="mt-1 block w-full"
                            value="{{ old('counter', 0) }}" required />
                        <p class="mt-1 text-xs text-gray-400">Akan bertambah otomatis setiap kali ada request baru dari
                            klien ini.</p>
                    </div>
                    ...
                    <div>
                        ...
                        <x-text-input id="name" name="name" type="text" class="mt-1 block w-full"
                            value="{{ old('name') }}" placeholder="Nama lengkap instansi klien" />
                    </div>
